...
export default function DateDetailClient({ date }: { date: string }) {
  ...
  const byKey: Record<string, { def: ReturnType<typeof getMinistry>; items: Item[] }> = {};
  for (const item of items) {
    const mk = labelToKey(item.ministry);
    if (!byKey[mk]) byKey[mk] = { def: getMinistry(mk), items: [] };
    byKey[mk].items.push(item);
  }

  const sections = Object.values(byKey).sort((a, b) => b.items.length - a.items.length);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">{items.length}件の新着情報</p>
      </div>
        ...
        {sections.map(({ def, items: ministryItems }) => (
          ...
          >
            ...
            >
              ...
              <span>{def.label}</span>
              <span className="text-xs text-gray-400">{ministryItems.length}件</span>
            </div>
                ...
                <li key={item.id} className="px-4 py-3 hover:bg-gray-50">
                  <div className="flex items-start gap-3">
                    <span className="text-xs text-gray-400 mt-0.5 min-w-[3rem]">
                      {item.published_at ? formatTimeJST(new Date(item.published_at)) : "--:--"}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-400 mt-1">{item.source_name}</div>
                    </div>
                  </div>
                </li>
              ))}
